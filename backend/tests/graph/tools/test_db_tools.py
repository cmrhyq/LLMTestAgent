...
覆盖 search_space / get_space_endpoints 的 Service 调用路径，
...
class TestSearchSpace:
    def test_search_fallback_when_no_match(self):
        """按名称搜索无结果时，fallback 查询启用空间且不抛异常。"""
        space = MagicMock()
        space.id = 1
        space.name = "demo"
        ...
        service = MagicMock()
        service.list_spaces.side_effect = [([], 0), ([space], 1)]
        ...
        with (
            ...
            patch("src.graph.tools.db_tools.SpaceService", return_value=service),
        ):
            mock_manager.return_value.get_session.return_value = ctx
            from src.graph.tools.db_tools import search_space

            result = search_space.invoke({"name": "不存在的空间"})

        assert service.list_spaces.call_count == 2
    ...
    def test_search_returns_match_directly(self):
        ...
        service = MagicMock()
        service.list_spaces.return_value = ([space], 1)
        ...
        with (
            ...
            result = search_space.invoke({"name": "real"})

        service.list_spaces.assert_called_once_with("real", None, 1, 100)
        assert json.loads(result)[0]["name"] == "real"


class TestGetSpaceEndpoints:
    def test_returns_endpoints_via_service(self):
        ...
        with (
            ...
            mock_manager.return_value.get_session.return_value = ctx
            from src.graph.tools.db_tools import get_space_endpoints

            result = get_space_endpoints.invoke({"space_id": 1})
