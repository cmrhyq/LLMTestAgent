...
set -euo pipefail
if ! git ls-files --error-unmatch backend/uv.lock >/dev/null 2>&1; then
    echo "ERROR: backend/uv.lock is not tracked. Check .gitignore for an over-broad rule excluding it." >&2
    exit 1
fi
