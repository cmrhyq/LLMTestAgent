...
本文描述 **TestAgents** 系统的完整工作流程——从用户输入自然语言指令开始，经由意图识别分流至「文档解析」或「测试执行」两条主线，最终汇聚于结果输出。
...
if (Intent?) then (parse_openapi)
  ...
  :Persist to Database (Space + Endpoints);
else (run_test)
...
| 2  | 发出 tool_calls | 调用 `search_space` 或 `get_space_endpoints` |
