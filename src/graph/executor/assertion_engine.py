...
class AssertionEngine:
    ...
    def evaluate_all(
        ...
        if all_passed:
            logger.debug(f"断言全部通过, rule_count: {len(rules)}", rule_count=len(rules))
        else:
            logger.debug(f"断言部分失败, total: {len(rules)}, failed: {failed_count}", total=len(rules), failed=failed_count)
    ...
    def _evaluate_single(
        ...
        try:
            ...
            logger.warning(f"断言评估异常, rule: {rule_str}, error: {e}", rule=rule_str, error=str(e))
            return {
                "rule": rule_str,
                "passed": False,
                "actual": None,
    ...
    def _compare(self, actual: Any, operator: str, expected: Any) -> bool:
        ...
        logger.warning(f"未知运算符, operator: {operator}", operator=operator)
        return False
