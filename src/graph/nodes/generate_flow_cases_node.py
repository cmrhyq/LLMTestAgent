...
def generate_flow_cases_node(state: AgentState) -> dict:
    ...
    with get_db_manager().get_session() as session:
        ...
        logger.info(f"LLM 流程用例响应长度: {len(response_text)}")
        ...
        for idx, case_data in enumerate(cases_data, 1):
            ...
            if not endpoint:
                logger.warning(f"步骤 {idx} 引用了未知的 endpoint_id={endpoint_id}，跳过")
                continue
...
def _parse_flow_response(response: str) -> List[Dict[str, Any]]:
    ...
    if code_block_match:
        ...
        if json_start == -1 or json_end <= 0:
            logger.warning("LLM 流程响应中未找到 JSON 内容")
            return []
    ...
    try:
        ...
        logger.error(f"解析 LLM 流程 JSON 响应失败: {e}")
        return []
